  ...
  if(
    ...
    !in_array($mybb->user['usergroup'], explode(',', $mybb->settings['lock_exempt']))
      ...
      if($mybb->user['uid'] == 0) {
        ...
        $return = "你必须 <a href=\"{$mybb->settings['bburl']}/member.php?action=register\">注册</a> 或者 <a href=\"{$mybb->settings['bburl']}/member.php?action=login\">登录</a> 才能查看.";
      ...
      // if they are logged in, but the item has a price that they haven't paid yet, tell them how they can pay for it.
        ...
        $return = "<form method=\"post\">
          <button type=\"submit\">支付 {$cost} 个顺德币.</button>
